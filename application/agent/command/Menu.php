...
class Menu extends Command
{
    ...
    protected function importRule($controller)
    {
        ...
        $className = "\\app\\agent\\controller\\" . implode("\\", $controllerArr) . $classSuffix;

        $pathArr = $controllerArr;
        array_unshift($pathArr, '', 'application', 'agent', 'controller');
        ...
        $className = "\\app\\agent\\command\\" . $uniqueName;
        //反射机制调用类的注释和方法名
        ...
        if (isset($matches[1]) && isset($matches[1][0]) && $matches[1][0]) {
            \think\Request::instance()->module('agent');
